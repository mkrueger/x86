...
/// Restore the JavaScript v86 PIC snapshot layout (13 bytes per controller).
/// The caller supplies master and slave fields in the exact order emitted by
/// `CPU.prototype.get_state_pic`.
pub fn restore_state(master: &[u8; 13], slave: &[u8; 13]) {
    let mut pic = get_pic();
    restore_pic0(&mut pic.master, master);
    restore_pic0(&mut pic.slave, slave);
}

fn restore_pic0(pic: &mut Pic0, state: &[u8; 13]) {
    pic.irq_mask = state[0];
    pic.irq_map = state[1];
    pic.isr = state[2];
    pic.irr = state[3];
    pic.master = state[4] != 0;
    pic.dummy = state[5];
    pic.expect_icw4 = state[6] != 0;
    pic.state = state[7];
    pic.read_isr = state[8] != 0;
    pic.auto_eoi = state[9] != 0;
    pic.elcr = state[10];
    pic.irq_value = state[11];
    pic.special_mask_mode = state[12] != 0;
}
